...
final class FunctionalInterfaceBuilder extends ClassSourceBuilder {
    ...
    private void emitFunctionalInterfaceMethod() {
        appendIndentedLines(STR."""
            \{Utils.methodTypeFor(funcType).returnType().getSimpleName()} apply(\{paramExprs("")});
            """);
    }
    ...
    private void emitDescriptorDecl() {
        appendIndentedLines(STR."""
            FunctionDescriptor $DESC = \{LayoutUtils.functionDescriptorString(0, funcType, runtimeHelperName())};
            """);
    }
}
